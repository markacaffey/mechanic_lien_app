...
def dashboard():
    ...
    # ✅ Open Liens (no sale date, not canceled)
    cursor.execute("""
...
def register():
    if request.method == "POST":
        ...
        role = request.form.get("role", "dealer")  # Default to 'dealer'
        ...
        try:
            cursor.execute(
                "INSERT INTO users (pnumber, email, password_hash, role) VALUES (?, ?, ?, ?)",
                (pnumber, email, hashed_password, role)
            )
            conn.commit()
...
def login():
    if request.method == "POST":
        ...
        # Use proper SQLite mode and URI
        ...
        # 🔍 Inspect the actual schema at runtime
        ...
        # Manual match — use explicit fields to be safe
        cursor.execute("SELECT id, pnumber, email, password_hash, role, dealer_id FROM users")
        ...
        for row in all_users:
            ...
            if row_pnumber == pnumber:
                print("✅ MATCH FOUND!")
                user = dict(row)  # ✅ Convert Row to dict for .get()
        ...
        if user:
            ...
            if check_password_hash(user["password_hash"], password):
                # ✅ Handle account status safely
                account_status = user.get("account_status", "active")
                if account_status != "active":
                    flash("⚠ Your account is inactive. Please contact support or update your billing info.", "danger")
                    return redirect(url_for("login"))
                ...
                return redirect(url_for("dashboard") if user["role"] == "admin" else url_for("select_dealer_for_vin"))
            else:
